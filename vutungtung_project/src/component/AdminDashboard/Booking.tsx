  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Handle the response properly - Axios wraps the response
        // response is AxiosResponse<Booking[]>, so response.data is Booking[]
        // But backend might return { data: Booking[] }
        const bookingsArray: Booking[] = Array.isArray(response.data)
          ? response.data
  ...
  const handleCancelBooking = async (bookingId: number, bookingCreatedAt: string) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const message = (response.data as any)?.message || "Failed to cancel booking.";
        setError(message);
      }
    } catch (err: any) {
      console.error("Error canceling booking:", err);
  ...
  const handleConfirmPayment = async (bookingId: number) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const message = (response.data as any)?.message || "Failed to confirm payment.";
        setError(message);
      }
    } catch (err: any) {
      console.error("Error confirming payment:", err);
