  ...
  const validateStep = () => {
    ...
    if (step === 2)
      return (
        auth?.user?.email &&
        auth?.user?.name &&
        (licenseFile || pending?.licenseImgBase64)
      );
                  ...
                  {paymentInfo.method === "esewa" && paymentInfo.agreed && (
                    <ESewaForm
                      ...
                      onBeforeSubmit={async () => {
                        ...
                        if (!licenseFile && !pending?.licenseImgBase64)
                          return false;
                        try {
                          ...
                          const url = `https://vutungtungrental-backend.onrender.com/vehicle/book/booking/${vehicle.categoryId}/${vehicle.id}`;
